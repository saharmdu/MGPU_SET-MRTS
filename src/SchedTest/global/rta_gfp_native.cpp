...
RTA_GFP_native::RTA_GFP_native()
    : GlobalSched(false, RTA, FIX_PRIORITY, NONE, "", "native") {}

RTA_GFP_native::RTA_GFP_native(TaskSet tasks, ProcessorSet processors,
...
ulong RTA_GFP_native::workload(const Task& task, ulong interval) {
  return task.get_wcet() * ceiling(interval, task.get_period()) +
         task.get_wcet();
}

ulong RTA_GFP_native::response_time(const Task& ti) {
